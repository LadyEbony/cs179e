...
public class ProgramInitializer extends GJDepthFirst<String, ClassEnvironment>{
	...
	void addMethodField(MethodEnvironment env, String id, String type){
		if (!env.addField(id, type)){
			if (errorMessage == null){
				errorMessage = "Error: Duplicate class method variable identifiers \"" + id + "\"";
			}
		}
	}
	...
	public String visit(VarDeclaration n, ClassEnvironment env){
		...
		// depth = 0 means class variables
		// otherwise function temporary variables
		if (depth == 0)
			addField(env, id, type);
		else 
			addMethodField(methodEnvironment, id, type);
	...
	public String visit(MethodDeclaration n, ClassEnvironment env){
		...
		methodEnvironment = env.getMethod(id);
